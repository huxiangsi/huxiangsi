  ...
  <div>
    ...
    <el-card class="box-card">
      ...
      <el-row :gutter="20">
        ...
        <el-col :span="3">
          <!-- 添加用户按钮 插槽 -->
          <slot name="addUserButton"><addUserButton @updateUserList="getUsersList"></addUserButton></slot>
        </el-col>
      </el-row>
      <!-- 表单数据展示区 -->
      <el-table style="width: 100%" :data="usersListData" border stripe>
        ...
        <el-table-column label="操作" width="180px">
          <template slot-scope="scope">
            <!-- 修改用户信息插槽 -->
            <slot name="editUserInfo"><editUserInfo :userInfo='scope.row' @editUsersInfoEvent='getUsersList'></editUserInfo></slot>
            <!-- 删除用户 插槽 -->
            <slot name="deleteUser"><deleteUser :sureDeleteUser='scope.row' @sureDeleteUserEvetn='getUsersList'></deleteUser></slot>
...
export default {
  ...
  components:{
    ...
    userStatuChanged(userInfo){
      ...
      this.$menu.put('users/'+userInfo.id + '/state/' + userInfo.mg_state).then(res => {
        if(res.data.meta.status !== 200){
